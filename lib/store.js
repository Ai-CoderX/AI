...
const saveMessage = async (message) => {
  ...
  // Skip if any required field is missing or if it's status/broadcast/newsletter
  if (!id || !jid || !message) return;
  
  // Check if it's a broadcast, newsletter, or status message
  if (isJidBroadcast(jid) || isJidNewsletter(jid) || jid === 'status@broadcast') {
    return;
  }
...
const getChatSummary = async () => {
  ...
  // Filter out broadcast, newsletter, and status messages
  const filteredMessages = messages.filter(msg => {
    const jid = msg.jid;
    return !isJidBroadcast(jid) && !isJidNewsletter(jid) && jid !== 'status@broadcast';
  });
  
  const distinctJids = [...new Set(filteredMessages.map((msg) => msg.jid))];

  const summaries = await Promise.all(
    distinctJids.map(async (jid) => {
      const chatMessages = filteredMessages.filter((msg) => msg.jid === jid);
