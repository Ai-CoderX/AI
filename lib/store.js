...
const shouldIgnoreJid = (jid) => {
  if (!jid) return true;
  if (isJidGroup(jid) || isJidBroadcast(jid) || isJidNewsletter(jid)) return true;
...
const saveMessage = async (message) => {
  ...
  // Don't save messages sent by the bot
  ...
  // Keep only last 40 messages for this chat
  const limitedChatMessages = chatMessages
    .sort((a, b) => b.timestamp - a.timestamp) // Sort by newest first
    .slice(0, 40); // Keep only 40 most recent
