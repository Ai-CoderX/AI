const config = require('../config');

const reactions = ['🌼','❤️','💐','🔥','🏵️','❄️','🧊','🐳','💥','🥀','❤‍🔥','🥹','😩','🫣','🤭','👻','👾','🫶','😻','🙌','🫂','🫀','👩‍🦰','🧑‍🦰','👩‍⚕️','🧑‍⚕️','🧕','👩‍🏫','👨‍💻','👰‍♀️','🦹🏻‍♀️','🧟‍♀️','🧟','🧞‍♀️','🧞','🙅‍♀️','💁‍♂️','💁‍♀️','🙆‍♀️','🙋‍♀️','🤷','🤷‍♀️','🤦','🤦‍♀️','💇‍♀️','💇','💃','🚶‍♀️','🚶','🧶','🧤','👑','💍','👝','💼','🎒','🥽','🐻','🐼','🐭','🐣','🪿','🦆','🦊','🦋','🦄','🪼','🐋','🐳','🦈','🐍','🕊️','🦦','🦚','🌱','🍃','🎍','🌿','☘️','🍀','🍁','🪺','🍄','🍄‍🟫','🪸','🪨','🌺','🪷','🪻','🥀','🌹','🌷','💐','🌾','🌸','🌼','🌻','🌝','🌚','🌕','🌎','💫','🔥','☃️','❄️','🌨️','🫧','🍟','🍫','🧃','🧊','🪀','🤿','🏆','🥇','🥈','🥉','🎗️','🤹','🤹‍♀️','🎧','🎤','🥁','🧩','🎯','🚀','🚁','🗿','🎙️','⌛','⏳','💸','💎','⚙️','⛓️','🔪','🧸','🎀','🪄','🎈','🎁','🎉','🏮','🪩','📩','💌','📤','📦','📊','📈','📑','📉','📂','🔖','🧷','📌','📝','🔏','🔐','🩷','❤️','🧡','💛','💚','🩵','💙','💜','🖤','🩶','🤍','🤎','❤‍🔥','❤‍🩹','💗','💖','💘','💝','❌','✅','🔰','〽️','🌐','🌀','⤴️','⤵️','🔴','🟢','🟡','🟠','🔵','🟣','⚫','⚪','🟤','🔇','🔊','📢','🔕','♥️','🕐','🚩','🇵🇰'];

const AutoReact = async (conn, mek, m, {
    from,
    body,
    isGroup,
    sender,
    senderNumber,
    botNumber,
    isReact,
    groupAdmins,
    isBotAdmins,
    isAdmins
}) => {
    try {
        if (!mek || !mek.message || mek.key.fromMe) return;
        if (isReact) return; // Skip reaction messages
        
        const userName = sender.split('@')[0];
        const botUserName = botNumber.split('@')[0] || botNumber;
        
        // Mode: 'owner' - only react to bot's own messages
        if (config.AUTO_REACT === 'owner') {
            if (!config.OWNER_REACT || senderNumber !== botNumber) return;
            const random = reactions[Math.floor(Math.random() * reactions.length)];
            await conn.sendMessage(from, {
                react: {
                    text: random,
                    key: mek.key,
                },
            });
            return;
        }
        
        // For all other modes, skip if sender is bot itself
        if (senderNumber === botNumber) return;

        // Mode: 'true' - react to everyone (except bot)
        if (config.AUTO_REACT === 'true') {
            const random = reactions[Math.floor(Math.random() * reactions.length)];
            await conn.sendMessage(from, {
                react: {
                    text: random,
                    key: mek.key,
                },
            });
            return;
        }

        // Mode: 'custom' - use custom emoji list (except bot)
        if (config.AUTO_REACT === 'custom') {
            const def = ['🥲','😂','👍🏻','🙂','😔'];
            const list = config.CUSTOM_REACT_EMOJIS ? 
                config.CUSTOM_REACT_EMOJIS.split(',').map(e => e.trim()) : def;
            const random = list[Math.floor(Math.random() * list.length)];
            await conn.sendMessage(from, {
                react: {
                    text: random,
                    key: mek.key,
                },
            });
            return;
        }

        // Mode: 'inbox' - only react to PMs (except bot)
        if (config.AUTO_REACT === 'inbox') {
            if (isGroup) return;
            const random = reactions[Math.floor(Math.random() * reactions.length)];
            await conn.sendMessage(from, {
                react: {
                    text: random,
                    key: mek.key,
                },
            });
            return;
        }

        // Mode: 'group' - only react in groups (except bot)
        if (config.AUTO_REACT === 'group') {
            if (!isGroup) return;
            const random = reactions[Math.floor(Math.random() * reactions.length)];
            await conn.sendMessage(from, {
                react: {
                    text: random,
                    key: mek.key,
                },
            });
            return;
        }

    } catch (error) {
        // Silent error handling
    }
};

module.exports = AutoReact;
