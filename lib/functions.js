...
// Enhanced isGroupAdmin function
...
// New function to get participant info
const getParticipantInfo = async (socket, jid, user) => {
    try {
        const groupMetadata = await socket.groupMetadata(jid);
        const participant = groupMetadata.participants.find(p => p.id === user);
        return participant || {};
...
// New function to check if user is real admin (superadmin)
const isRealAdmin = async (socket, jid, user) => {
    try {
        const participant = await getParticipantInfo(socket, jid, user);
        return participant?.admin === 'superadmin' || false;
    } catch (error) {
        console.error('[ ❌ ] isRealAdmin error:', error);
        return false;
    }
};
