...
const sms = async (conn, m, store) => {
    // Dynamically import Baileys
    const baileys = await import("@whiskeysockets/baileys");
    const { proto, getContentType, BufferJSON } = baileys;
    ...
    // FIXED: Use decodeAndHydrate() as recommended by Baileys
    m.copy = () => {
        try {
            // First try with encode/decodeAndHydrate (recommended by Baileys)
            if (typeof M.encode === 'function') {
                const buffer = M.encode(m).finish();
                // Use decodeAndHydrate() instead of decode() for proper hydration
                const copy = M.decodeAndHydrate ? M.decodeAndHydrate(buffer) : M.decode(buffer);
                return sms(conn, copy, store);
            } else {
                // Fallback to BufferJSON serialization
                const json = JSON.stringify(m, BufferJSON.replacer);
                const data = JSON.parse(json, BufferJSON.reviver);
                const copy = M.create(data);
                return sms(conn, copy, store);
            }
        } catch (error) {
            ...
            try {
                const copy = M.create(JSON.parse(JSON.stringify(m)));
                return sms(conn, copy, store);
            } catch (e) {
                console.error('Even fallback failed:', e);
                return m; // Return original if all fails
            }
        }
    }
