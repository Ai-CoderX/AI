...
const getOriginalMessageContent = (mek) => {
    if (mek.message?.protocolMessage?.editedMessage?.conversation) 
        return mek.message.protocolMessage.editedMessage.conversation;
    if (mek.message?.protocolMessage?.editedMessage?.extendedTextMessage?.text) 
        return mek.message.protocolMessage.editedMessage.extendedTextMessage.text;
...
const EditedText = async (conn, mek, jid, editInfo, isGroup) => {
    const editedText = getMessageContent(mek);
    const originalText = getOriginalMessageContent(mek);
...
// Main AntiDelete function that handles BOTH delete and edit detection
const AntiDelete = async (conn, updates) => {
    // Check if ANTI_DELETE is enabled - if not, exit immediately
    ...
    for (const update of updates) {
            ...
            if (store && store.message) {
                ...
                if (isGroup) {
                    try {
                        ...
                        // Use RECOVERY_PATH from config
                        jid = config.RECOVERY_PATH === "inbox" ? conn.user.id.split(':')[0] + "@s.whatsapp.net" : store.jid;
                    } catch (e) {
                    ...
                    // Use RECOVERY_PATH from config
                    jid = config.RECOVERY_PATH === "inbox" ? conn.user.id.split(':')[0] + "@s.whatsapp.net" : update.key.remoteJid || store.jid;
                }
        ...
        // === CHECK FOR EDITED MESSAGES (only if ANTI_EDIT is enabled) ===
        if (config.ANTI_EDIT === "true" && update.update.message?.protocolMessage?.editedMessage) {
            const mek = update.update.message;
            
            // Skip if message was edited by the bot
            if (mek.key?.fromMe) continue;
            ...
            if (isGroup) {
                ...
                // Use RECOVERY_PATH from config
                jid = config.RECOVERY_PATH === "inbox" ? conn.user.id.split(':')[0] + "@s.whatsapp.net" : update.key.remoteJid;
            } else {
                const senderNumber = mek.key?.participant?.split('@')[0] || mek.key?.remoteJid?.split('@')[0] || 'Unknown';
                editInfo = `*╭────⬡ KHAN-MD ⬡────*\n*├▢ EDITOR :* @${senderNumber}\n*├▢ ACTION :* Edited a Message`;
                // Use RECOVERY_PATH from config
            ...
            await EditedText(conn, mek, jid, editInfo, isGroup);
        }
    }
};
...
module.exports = {
    ...
    EditedText,
    DeletedMedia,
    AntiDelete, // Only export AntiDelete - it handles everything
};
